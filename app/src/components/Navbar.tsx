      ...
      <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4">
        <div className="bg-slate-900/80 backdrop-blur-xl rounded-full px-8 py-3 border border-slate-800/50 shadow-2xl w-full md:w-fit max-w-2xl">
          <div className="flex items-center justify-between gap-8 md:gap-12">
            <motion.div whileHover={{ scale: 1.05 }} className="font-bold text-lg bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-cyan-300 shrink-0">
              PORTFOLIO
            </motion.div>
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden text-slate-400">
              {isMenuOpen ? <X size={20} /> : <div className="w-5 h-0.5 bg-slate-400 rounded-full"></div>}
            </motion.button>
          </div>
        </div>
      </motion.nav>
              ...
              <div className="flex flex-col space-y-4">
                {navItems.map((item) => (
                  <motion.button
                    ...
                    whileHover={{ x: 5 }}
                    onClick={() => handleScroll(item.ref)}
                    className="text-left px-4 py-3 text-slate-300 hover:text-white hover:bg-slate-800/50 rounded-lg transition-colors font-medium text-lg"
                  >
                    {item.label}
                  </motion.button>
                ))}
